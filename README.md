...
├── guardian-app/           # the guardian's companion app (separate Vite project)
└── frontend/               # VIEW — React + TypeScript SPA, also the Android app
    └── src/
...
The same React client, wrapped by [Capacitor](https://capacitorjs.com) into
an APK a mother or a clinician installs — the backend is the one above,
unchanged. Needs Android Studio (for its SDK and JDK; nothing else is read
from it).
...
Builds `frontend/dist-native`, syncs it into `frontend/android`, runs Gradle,
and puts the result at `public/downloads/maternalcare.apk` — which the
running server also serves at `/downloads/maternalcare.apk`. The sign-in
screen in the app asks for the **server address** (`192.168.0.12:3000` for a
laptop on the same wifi, or a deployed host) and remembers it; pass
`-- --api https://host/api` to compile one in as the default. iOS is the
same code with `npx cap add ios`, but building it needs a Mac with Xcode.
...
side (`frontend/android/…/MaternalCareWidget.java`) works out the week and
...
Deploying: set `NODE_ENV=production` behind a TLS-terminating proxy, build the
client (`npm --prefix frontend run build`) so this process serves it on one
origin, and leave `CLIENT_ORIGIN` unset unless the client is hosted elsewhere.
